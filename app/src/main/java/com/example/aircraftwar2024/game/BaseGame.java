...
public abstract class BaseGame extends SurfaceView implements SurfaceHolder.Callback, Runnable{
    ...
    public BaseGame(Context context){
        ...
        music = new MyMediaPlayer(context);
        sound = new MySoundPool(context);
            ...
            for (AbstractEnemyAircraft enemyAircraft : enemyAircrafts) {
                ...
                if (enemyAircraft.crash(bullet)) {
                    ...
                    if (enemyAircraft.notValid()) {
                        ...
                        if(enemyAircraft instanceof BossEnemy&&needMusic){
                            music.pauseBossBgm();
                            music.playBgm();
                        }
                    }
    ...
    private void paintScoreAndLife() {
        ...
        String strScore = "Score:"+this.getScore();
        String strLife = "Life:"+heroAircraft.getHp();
        canvas.drawText(strScore,20,80,paint);
        canvas.drawText(strLife,20,180,paint);
        if(MainActivity.isOnline){
//            canvas.drawText("NAME:" + OnlineActivity.getOpponentName(), 500, 80, paint);
            canvas.drawText("SCORE:" + OnlineActivity.getOpponentScore(), 500, 180, paint);
        }
    }
